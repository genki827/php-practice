...
- 加算あるいは減算したDate Timeオブジェクト型を返します。失敗した場合に false を返す。
...
-formatは→でつなげているため、diffの表示の仕方を指定してる。
 同様にdiffは$nowと$prevの差を指定しているのでそこから逆算してDateTimeのdiffなので日付の差=DateIntervalのformatを調べる。